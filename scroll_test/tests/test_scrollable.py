...
elements_under_test = (
        {'id': 'Vertical scroll', 'result': True, 'page': 'element_with_vertical_scroll.html'},
        {'id': 'Horizontal scroll', 'result': True, 'page': 'element_with_horizontal_scroll.html'},
...
pages_under_test = (
        ...
        {'id': 'Horizontal scroll', 'result': True, 'page': 'page_with_horizontal_scroll.html'},
...
def is_element_scrollable_var_1(driver, element):
        print(driver.execute_script('return arguments[0].scrollHeight > arguments[0].offsetHeight;', element))
        return driver.execute_script('return arguments[0].scrollHeight > arguments[0].offsetHeight;', element)
...
def is_element_scrollable_var_2(element):
        return is_element_horizontal_scrollable(element) or is_element_vertical_scrollable(element)


def is_page_horizontal_scrollable(driver):
    return driver.execute_script("return document.documentElement.scrollWidth >"
...
@pytest.fixture(params=elements_under_test, ids=[f'{c["id"]}' for c in elements_under_test])
